...
## まとめ
どうせプロパティ存在しなかったらどこかでエラー出るから前者のキャストでいい気もする
...
Dateをsetすると、Dateが`firebase.firestore.Timestamp`に勝手に変換されることがわかる。

Date->Timestampと勝手に変換されるということは、変換処理が必要になる。これの対応案
...
```
みたいに、`firebase`と`firebase-admin`でconverterを使いまわせる（動作確認はしてない）。
...
export const getUserDoc = (uid: string) => {
  firebase.firestore().doc(`uid/${uid}`).withConverter(userConverter);
};
```

これに加えて、DateをNumberで保存すれば良い感じになるはず。

見てのとおりfirestoreは型付けるの大変だし、書き込み速度が毎秒1回くらいに制限されるし、ライブラリはバカ重い(目下改善中らしい)ので結構使いにくい。使い方を選びましょう。
