...
node-semverでは`^1.2.3`や`~1.2.3`, `>1.2.3`というようにバージョンの範囲を表します。これはSemVerの仕様外であるため、他の実装に持って行ったときに期待通り動く保証はどこにもありません。
...
の５つの記号を使用できます。例えば`>=1.2.7`などです。わかりやすいですね。ちなみに`1.2.3`とかくと`=1.2.3`のことだと解釈されます。等号はあってもなくてもいいです。
...
なんとこれは**どちらも成り立ちます**。前述した通り「省略した桁はなんでも良い」ので、`<=1`は実質的に`<=1.999.999`になります ^[わかりやすさのため999で桁の最大値を表しています]。したがって1番目の不等式が成り立ちます。
...
## レキサーの生成
Alexは`.x`ファイルに文法を定義することでレキサーを生成します。AlexにはWrapperといって、Alexが提供するミニマルなAPIをラップして様々なインターフェースのレキサーを生成するオプションがあります。ミニマルなAPIは以下のような型を持ちます。
...
こちらがレキサーの定義です。左側にあるのがトークンにマッチする正規表現で、右側の`{}`の中にあるのが先ほどの`action`型である任意のHaskellコードです。
...
2行目の定義が`-xxxx`のような識別子をトークン化する定義です。`x`がバージョン番号にも識別子にも使えるので、`[\-\+]^`を前につけることで`-`, `+`以降のみに当てはまるルールであると定義します。かなり便利な機能です。

node-semverは空白の有無が重要なので読み飛ばさずにトークン化します。

## パーサーの生成

happyは`.y`拡張子の文法定義からパーサーを生成します。`%monad`と`%lexer`を設定すると、happyの生成するパーサーの型は以下になります。
...
`parse`はHappyの生成する構文解析器です。Alex型は名前からして字句解析のためのモナドの印象が強いですが、実際には「パーサーとレキサーで必要なもの全般」を放り込んでおくモナドです。Alexモナドを使ってレキサーとパーサーで情報をやり取りすることができます。

`parseError`はわかりやすいと思います。今見ているトークンとパーサーの状態を参照してエラーを返します。

`lexer`はそのままレキサーですが、型が直感的ではないですね。第一引数は"１トークン受け取ってそれを消費するパーサー”です。これはhappyが渡します。ユーザーは`lexer`関数の中でトークンを1つパースし、第一引数に渡します。このトークンをパースする過程でLexerの状態を変更して文脈に依存した解析などをすることができます。happyは終了トークンが渡されるまで`lexer`を呼んで構文解析を行います。
...
この文法は`>1.2.3 || 2.4`のようなORでつなげられた文法を表します。右の`{}`内に任意のHaskellコードを書くことができます。文法がパースできたときにこのコードが実行されます。
`$1`のような記号は左側の文法定義の各項を表します。図示するとこんな感じです。
```
>1.2.3   ||    2.4
^$1     ^$2    ^$3
```
...
# まとめ
この記事では SemVerとnode-semverの違いを紹介し、AlexとHappyを使ったパーサーを実装について少しだけ触れました。Rustが盛り上がってきていますが、実はHaskellも同じ系統に属しています。良い言語なので触ってみてください。
